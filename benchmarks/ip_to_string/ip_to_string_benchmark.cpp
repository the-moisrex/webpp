...
static char const* apple_inet_ntop6(const struct apple_in6_addr* addr, char* dst, size_t size) {
    ...
    if ((i != 0) || (j != 0)) {
        ...
        len = snprintf(tmp, sizeof(tmp), "::%s%s", (i != 0) ? "ffff:" : "", apple_inet_ntop4(&a4, tmp2, sizeof(tmp2)));
        if (len >= size) {
            return NULL;
        }
        bcopy(tmp, dst, len + 1);
...
constexpr void short_str_to(octets_t const& octets, auto& buffer) noexcept {
    ...
    typename decltype(_octets)::const_iterator range_start = _octets.cend(), range_end = _octets.cend(), start,
...
namespace v2 {
    ...
    [[nodiscard]] constexpr bool is_v4_mapped(stl::uint8_t const* octets) noexcept {
        return (octets[0] == 0) && (octets[1] == 0) && (octets[2] == 0) && (octets[3] == 0) && (octets[4] == 0) &&
               (octets[5] == 0) && (octets[6] == 0) && (octets[7] == 0) && (octets[8] == 0) && (octets[9] == 0) &&
