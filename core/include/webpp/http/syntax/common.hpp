...
namespace webpp::http {
    ...
    template <istl::StringView StrViewType = decltype(http_lws)>
    [[nodiscard]] static inline auto ltrim_copy_lws(istl::ConvertibleToStringView auto&& s) noexcept {
        return ascii::ltrim_copy<StrViewType>(stl::forward<decltype(s)>(s), http_lws);
    }

    // trim from end (copying)
    template <istl::StringView StrViewType = decltype(http_lws)>
    [[nodiscard]] static inline auto rtrim_copy_lws(istl::ConvertibleToStringView auto&& s) noexcept {
        return ascii::rtrim_copy<StrViewType>(stl::forward<decltype(s)>(s), http_lws);
    }

    // trim from both ends (copying)
    template <istl::StringView StrViewType = decltype(http_lws)>
    [[nodiscard]] static inline auto trim_copy_lws(istl::ConvertibleToStringView auto&& s) noexcept {
    ...
    // trim from start (copying)
    template <istl::StringView StrViewType = decltype(http_lws)>
    [[nodiscard]] static inline auto ltrim_copy_lws(istl::ConvertibleToString auto&& s, auto const& allocator) noexcept {
        return ascii::ltrim_copy<StrViewType>(stl::forward<decltype(s)>(s), allocator, http_lws);
    }

    // trim from end (copying)
    template <istl::StringView StrViewType = decltype(http_lws)>
    [[nodiscard]] static inline auto rtrim_copy_lws(istl::ConvertibleToString auto&& s, auto const& allocator) noexcept {
        return ascii::rtrim_copy<StrViewType>(stl::forward<decltype(s)>(s), allocator, http_lws);
    }

    // trim from both ends (copying)
    template <istl::StringView StrViewType = decltype(http_lws)>
    [[nodiscard]] inline auto trim_copy_lws(istl::ConvertibleToString auto&& s, auto const& allocator) noexcept {
