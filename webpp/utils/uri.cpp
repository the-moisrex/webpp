...
namespace {
     ...
     */
    constexpr webpp::charset ALPHA{webpp::charset('a', 'z'),
                                   webpp::charset('A', 'Z')};

    /**
     * This is the character set containing just numbers.
     */
    constexpr webpp::charset DIGIT('0', '9');

    /**
     * This is the character set containing just the characters allowed
     * in a hexadecimal digit.
     */
    const webpp::charset HEXDIG {
        {webpp::charset('0', '9'), webpp::charset('A', 'F'),
         webpp::charset('a', 'f')};
         ...
         */
        const webpp::charset UNRESERVED{{ALPHA, DIGIT, '-', '.', '_', '~'}};

        /**
         * This is the character set corresponds to the "sub-delims" syntax
         * specified in RFC 3986 (https://tools.ietf.org/html/rfc3986).
         */
        const webpp::charset SUB_DELIMS{
            {'!', '$', '&', '\'', '(', ')', '*', '+', ',', ';', '='}};
         ...
         * specified in RFC 3986 (https://tools.ietf.org/html/rfc3986).
         */
        const webpp::charset SCHEME_NOT_FIRST{
            ALPHA, DIGIT, '+', '-', '.',
        };
         ...
         */
        const webpp::charset PCHAR_NOT_PCT_ENCODED{UNRESERVED, SUB_DELIMS, ':',
                                                   '@'};
         ...
         * leaving out "pct-encoded".
         */
        const webpp::charset QUERY_OR_FRAGMENT_NOT_PCT_ENCODED{
         ...
         */
        const webpp::charset QUERY_NOT_PCT_ENCODED_WITHOUT_PLUS{
            UNRESERVED, '!', '$', '&', '\'', '(', ')', '*',
            ',',        ';', '=', ':', '@',  '/', '?'};
         ...
         * leaving out "pct-encoded".
         */
        const webpp::charset USER_INFO_NOT_PCT_ENCODED{
            UNRESERVED,
            SUB_DELIMS,
            ':',
        };
         ...
         * leaving out "pct-encoded".
         */
        const webpp::charset REG_NAME_NOT_PCT_ENCODED{UNRESERVED, SUB_DELIMS};
         ...
         * specified in RFC 3986 (https://tools.ietf.org/html/rfc3986).
         */
        const webpp::charset IPV_FUTURE_LAST_PART{UNRESERVED, SUB_DELIMS, ':'};
        ...
        bool ValidateIpv6Address(const std::string& address) {
            ...
            if ((position == address.length()) &&
                ((state == ValidationState::COLON_BUT_NO_GROUPS_YET) ||
                 ...
                 (state ==
                  ValidationState::COLON_AFTER_GROUP))) { // trailing single
                                                          // colon
                return false;
            }
            if (ipv4AddressEncountered) {
                if (!ValidateIpv4Adress(
                        address.substr(potentialIpv4AddressStart))) {
                    return false;
                }
                numGroups += 2;
            }
        ...
        std::string EncodeElement(const std::string& element,
            ...
            for (uint8_t c : element) {
                if (allowedCharacters.Contains(c)) {
                    ...
                    encodedElement.push_back(
                        MakeHexDigit((unsigned int)c >> 4));
                    encodedElement.push_back(
                        MakeHexDigit((unsigned int)c & 0x0F));
                }
            }
        ...
        bool DecodeQueryOrFragment(std::string & queryOrFragment) {
            return DecodeElement(queryOrFragment,
        ...
        struct Uri::Impl {
            ...
            bool ParseAuthority(const std::string& authorityString) {
                ...
                if (userInfoDelimiter == std::string::npos) {
                    ...
                    hostPortString =
                        authorityString.substr(userInfoDelimiter + 1);
                }

                // Next, parsing host and port from authority and path.
                std::string portString;
                HostParsingState hostParsingState =
                    HostParsingState::FIRST_CHARACTER;
                host.clear();
                ...
                for (const auto c : hostPortString) {
                    ...
                    case HostParsingState::IP_LITERAL: {
                        if (c == 'v') {
                            host.push_back(c);
                            hostParsingState =
                                HostParsingState::IPV_FUTURE_NUMBER;
                            break;
                        } else {
                    ...
                    case HostParsingState::IPV_FUTURE_NUMBER: {
                        if (c == '.') {
                            hostParsingState =
                                HostParsingState::IPV_FUTURE_BODY;
                        } else if (!HEXDIG.Contains(c)) {
                            return false;
                        }
                        host.push_back(c);
            ...
            bool ParseScheme(const std::string& uriString, std::string& rest) {
                ...
                const auto schemeEnd =
                    uriString.substr(0, authorityOrPathDelimiterStart)
                        .find(':');
             ...
             */
            bool
            SplitAuthorityFromPathAndParseIt(std::string authorityAndPathString,
                ...
                if (!relativeReference.impl_->host.empty()) {
                    ...
                    if (relativeReference.impl_->path.empty()) {
                        ...
                        if (relativeReference.impl_->IsPathAbsolute()) {
                            target.impl_->CopyAndNormalizePath(
                                relativeReference);
                        } else {
                            // RFC describes this as:
        ...
        std::string Uri::GenerateString() const {
            ...
            if (impl_->HasAuthority()) {
                ...
                if (!impl_->userInfo.empty()) {
                    buffer << EncodeElement(impl_->userInfo,
                ...
                if (!impl_->host.empty()) {
                    if (ValidateIpv6Address(impl_->host)) {
                        buffer << '['
                               << SystemAbstractions::ToLower(impl_->host)
                               << ']';
                    } else {
                        buffer << EncodeElement(impl_->host,
