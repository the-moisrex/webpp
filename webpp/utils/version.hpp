...
#undef WEBPP_DEFINE_OPERATOR
        ...
        constexpr void from_safe_string(StrT&& inp_str) noexcept {
            ...
            for (auto octet = stl::rbegin(*this); octet != stl::rend(*this); ++octet) {
                auto const dot = str.find('.');
                *octet         = to<integer_type>(str.substr(0, dot));
        ...
        from_string(StrT&& inp_str) noexcept {
            ...
            for (auto octet = stl::rbegin(*this); octet != stl::rend(*this); ++octet) {
                auto const dot = str.find('.');
                if (auto const value =
                      to<integer_type, 10, error_handling_strategy::use_expected>(str.substr(0, dot));
                    value) {
                    *octet = value.value();
                } else {
                    return false;
        ...
        // Array comparison
        [[nodiscard]] constexpr inline bool operator==(const basic_version& other) noexcept {
            return std::equal(this->begin(), this->end(), other.begin());
        }

#if __cpp_lib_three_way_comparison && __cpp_lib_concepts
        [[nodiscard]] constexpr auto operator<=>(const basic_version& other) noexcept {
            return as_array() <=> other.as_array();
        }

        // template <istl::StringViewifiable StrT>
        // [[nodiscard]] constexpr auto operator<=>(StrT&& other) noexcept {
        //     return as_array() <=> .as_array(); // todo
        // }
#endif
    };
