...
webpp::cookie_jar::const_iterator
webpp::cookie_jar::find(std::string const& name) const noexcept {
    return std::find_if(cbegin(), cend(),
                        [&](auto const& a) { return a.name() == name; });
}

webpp::cookie_jar::const_iterator
webpp::cookie_jar::find(const webpp::cookie& c) const noexcept {
    return std::find_if(cbegin(), cend(),
                        [&](auto const& a) { return a.same_as(c); });
}

void webpp::cookie_jar::make_unique(
    const webpp::cookie_jar::const_iterator& dont_touch,
...
void webpp::cookie_jar::change_if(
    webpp::cookie_jar::condition const& if_statement,
...
void webpp::cookie_jar::change_all(
    std::function<void(iterator&)> const& change) noexcept {
    for (auto it = begin(); it != end(); it++)
        change(it);
}

std::pair<webpp::cookie_jar::iterator, bool>
webpp::cookie_jar::insert(const value_type& value) {
...
std::pair<webpp::cookie_jar::iterator, bool>
webpp::cookie_jar::insert(value_type&& value) {
    auto found = find(value);
    if (found != cend())
        erase(found);
...
webpp::cookie_jar::iterator webpp::cookie_jar::insert(const_iterator hint,
...
void webpp::cookie_jar::insert(std::initializer_list<value_type> ilist) {
...
webpp::cookie_jar&
webpp::cookie_jar::encrypted(cookie::encrypted_t const& _encrypted) noexcept {
    for (auto& c : *this)
        c._encrypted = _encrypted;
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::encrypted(condition const& _condition,
                             cookie::encrypted_t const& _encrypted) noexcept {
    change_if(_condition, [&](auto& it) { it->_encrypted = _encrypted; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::encrypted(cookie::name_t const& _name,
                             cookie::encrypted_t const& _encrypted) noexcept {
    change_if(_name, [&](auto& it) { it->_encrypted = _encrypted; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::encrypted(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::secure(cookie::secure_t const& _secure) noexcept {
    for (auto& c : *this)
        c._secure = _secure;
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::secure(condition const& _condition,
                          cookie::secure_t const& _secure) noexcept {
    change_if(_condition, [&](auto& it) { it->_secure = _secure; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::secure(cookie::name_t const& _name,
                          cookie::secure_t const& _secure) noexcept {
    change_if(_name, [&](auto& it) { it->_secure = _secure; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::secure(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::host_only(cookie::host_only_t const& _host_only) noexcept {
    for (auto& c : *this)
...
webpp::cookie_jar&
webpp::cookie_jar::host_only(condition const& _condition,
...
webpp::cookie_jar&
webpp::cookie_jar::host_only(cookie::name_t const& _name,
                             cookie::host_only_t const& _host_only) noexcept {
...
webpp::cookie_jar&
webpp::cookie_jar::host_only(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::prefix(cookie::prefix_t const& _prefix) noexcept {
    for (auto& c : *this)
        c._prefix = _prefix;
    return *this;
}
...
webpp::cookie_jar&
webpp::cookie_jar::prefix(cookie::name_t const& _name,
...
webpp::cookie_jar&
webpp::cookie_jar::prefix(condition const& _condition,
                          cookie::prefix_t const& _prefix) noexcept {
...
webpp::cookie_jar&
webpp::cookie_jar::prefix(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::comment(cookie::comment_t const& _comment) noexcept {
    for (auto& c : *this)
        c._comment = _comment;
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::comment(condition const& _condition,
                           cookie::comment_t const& _comment) noexcept {
    change_if(_condition, [&](auto& it) { it->_comment = _comment; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::comment(cookie::name_t const& _name,
                           cookie::comment_t const& _comment) noexcept {
    change_if(_name, [&](auto& it) { it->_comment = _comment; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::comment(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::same_site(cookie::same_site_t const& _same_site) noexcept {
    for (auto& c : *this)
        c._same_site = _same_site;
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::same_site(cookie::name_t const& _name,
                             cookie::same_site_t const& _same_site) noexcept {
    change_if(_name, [&](auto& it) { it->_same_site = _same_site; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::same_site(condition const& _condition,
                             cookie::same_site_t const& _same_site) noexcept {
    change_if(_condition, [&](auto& it) { it->_same_site = _same_site; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::same_site(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::expires(cookie::date_t const& _expires) noexcept {
    for (auto& c : *this)
...
webpp::cookie_jar&
webpp::cookie_jar::expires(cookie::name_t const& _name,
...
webpp::cookie_jar&
webpp::cookie_jar::expires(condition const& _condition,
                           cookie::date_t const& _expires) noexcept {
...
webpp::cookie_jar&
webpp::cookie_jar::expires(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::max_age(cookie::max_age_t const& _max_age) noexcept {
    for (auto& c : *this)
        c._max_age = _max_age;
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::max_age(cookie::name_t const& _name,
                           cookie::max_age_t const& _max_age) noexcept {
    change_if(_name, [&](auto& it) { it->_max_age = _max_age; });
    return *this;
}
webpp::cookie_jar&
webpp::cookie_jar::max_age(condition const& _condition,
                           cookie::max_age_t const& _max_age) noexcept {
...
webpp::cookie_jar&
webpp::cookie_jar::max_age(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::value(cookie::value_t const& _value) noexcept {
    for (auto& c : *this)
        c._value = _value;
    return *this;
}
...
webpp::cookie_jar&
webpp::cookie_jar::value(cookie::name_t const& _name,
...
webpp::cookie_jar&
webpp::cookie_jar::value(const_iterator const& it,
                         cookie::value_t const& _value) noexcept {
    return value(it, cookie::value_t(_value));
}

webpp::cookie_jar& webpp::cookie_jar::value(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::value(condition const& _condition,
                         cookie::value_t const& _value) noexcept {
...
webpp::cookie_jar&
webpp::cookie_jar::path(cookie::path_t const& _path) noexcept {
    change_all([&](auto& it) {
        it->_path = _path;
...
webpp::cookie_jar&
webpp::cookie_jar::path(cookie::name_t const& _name,
...
webpp::cookie_jar&
webpp::cookie_jar::path(condition const& _condition,
                        cookie::path_t const& _path) noexcept {
    change_if(_condition, [&](auto& it) {
        it->_path = _path;
        make_unique(it, [&](auto const& c) { return c._path == _path; });
...
webpp::cookie_jar& webpp::cookie_jar::path(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::path(const_iterator const& it,
                        cookie::path_t const& _path) noexcept {
    return path(it, cookie::path_t(_path));
}

webpp::cookie_jar&
webpp::cookie_jar::domain(cookie::domain_t const& _domain) noexcept {
    change_all([&](auto& it) {
        it->_domain = _domain;
...
webpp::cookie_jar&
webpp::cookie_jar::domain(cookie::name_t const& _name,
...
webpp::cookie_jar&
webpp::cookie_jar::domain(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::domain(condition const& _condition,
                          cookie::domain_t const& new_domain) noexcept {
...
webpp::cookie_jar&
webpp::cookie_jar::name(cookie::name_t const& _name) noexcept {
    change_all([&](auto& it) {
        it->_name = _name;
...
webpp::cookie_jar&
webpp::cookie_jar::name(cookie::name_t const& old_name,
...
webpp::cookie_jar& webpp::cookie_jar::name(const_iterator const& it,
...
webpp::cookie_jar&
webpp::cookie_jar::name(const_iterator const& it,
                        cookie::name_t const& new_name) noexcept {
    return name(it, cookie::name_t(new_name));
}

webpp::cookie_jar&
webpp::cookie_jar::name(condition const& _condition,
                        cookie::name_t const& new_name) noexcept {
    change_if(_condition, [&](auto& it) {
        it->_name = new_name;
        make_unique(it, [&](auto const& c) { return c._name == new_name; });
